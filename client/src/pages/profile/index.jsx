...
function Profile() {
  ...
  const saveChanges = async () => {
    if(validateProfile()) {
      try {
        const res = await apiClient.post(UPDATE_PROFILE_ROUTE, { firstName, lastName, color: selectedColor},
      ...
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        <div className='grid grid-cols-2'>
          <div className='h-full w-32 md:w-48 md:h-48 relative flex items-center justify-center' onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <Avatar className='h-32 w-32 md:w-48 md:h-48 border-[1px] rounded-full overflow-hidden'>
              {image ? (
                <AvatarImage
                  src={image}
                  alt="profile"
